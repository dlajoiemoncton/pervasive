...
#'
#' @export
pervasive_dic_glm <- function(formula, data, min_support) {
  ...
  #There are many performance measures that might be used for a glm. It is best left to individual researchers.
  ...
  dichotomize <- function(x) {
    if (length(unique(x)) <= 2) {
      ...
      # Discretize for non-binary variables
      discretize(x, breaks = 2, labels = c("low", "high"),
                 include.lowest = TRUE, ordered_result = TRUE)
    }
  }

  # Apply the updated dichotomization function
...
psych::describe(data$sex)
  ...
  class(result) <- "pervasive_dic_glm"
  return(result)
}

#' @export
# Custom print function for pervasive object
print.pervasive_dic_glm <- function(x, ...) {
  cat("\nThe observed percentage of concordant pairs (OPCP) for the regression is ",
